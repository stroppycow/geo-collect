...
class CheckParentUnicityAfterDownloadInseeCog(DataValidationAndConsistencyInseeCog):
    def __init__(
            self,
            parent_unicity_exceptions: URIsUnicityInseeExceptionsToIgnoreOrCorrect
        ):
        super().__init__()
    ...
    def handle_exceptions(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection):
        ...
        uri_removals = [exception for exception in self.parent_unicity_exceptions.root if isinstance(exception, ErrorHandlerRemoveDuplicatedURIAfterDownloadInseeCog)]
