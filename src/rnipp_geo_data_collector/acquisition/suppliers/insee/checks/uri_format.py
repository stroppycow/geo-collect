...
from .error_handler import ErrorHandlerReplaceCheckURIAfterDownloadInseeCog, ErrorHandlerRemoveCheckURIAfterDownloadInseeCog


if TYPE_CHECKING:
    from ..requests import RequestCOG
    from ..config import URIFormatInseeExceptionsToIgnoreOrCorrect

class CheckURIAfterDownloadInseeCog(DataValidationAndConsistencyInseeCog):
    def __init__(
            self,
            uri_format_exceptions: URIFormatInseeExceptionsToIgnoreOrCorrect,
            pattern: str
        ):
        super().__init__()
        self.uri_format_exceptions = uri_format_exceptions
        self.pattern = pattern

    def handle_exceptions(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection):
        """Handle exceptions in URI format checks"""
        template_path = Path(__file__).parent.parent / "sql" / "uri_correct.mustashe.sql"
        renderer = pystache.Renderer(escape=lambda s: s) 
        
        modif_uri_sql = "uri"
        filter_condition = ""

        uri_updates = [exception for exception in self.uri_format_exceptions.root if isinstance(exception, ErrorHandlerReplaceCheckURIAfterDownloadInseeCog)]
        uri_removals = [exception for exception in self.uri_format_exceptions.root if isinstance(exception, ErrorHandlerRemoveCheckURIAfterDownloadInseeCog)]
        
        if uri_updates:
            modif_uri_sql = "CASE " + " ".join([f"WHEN uri = '{exception.uri}' THEN '{exception.replace}'" for exception in uri_updates]) + f" ELSE uri END"
    ...
    def run(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection, allow_exceptions: bool = False) -> bool:
        """Check if the content of the file is valid for the URI variable"""
        template_path = Path(__file__).parent.parent / "sql" / "uri_check.mustashe.sql"
        renderer = pystache.Renderer(escape=lambda s: s) 
        context: dict[str, str] = {
            "view_name": request.view_name,
        ...
        try:
            ...
            if len(data_bug) > 0:
                ...
                if allow_exceptions and uri_bug in [exception.uri for exception in self.uri_format_exceptions.root]:
            ...
            raise RuntimeError(f"Unexpected error while checking URI of {request.description} data after downloading") from e
                  
        
        logging.info(f"Successfully checked URI of {request.description} after downloading")
        return True
