...
from .error_handler import ErrorHandlerReplaceBadInseeCodeOverlapAfterDownloadInseeCog, ErrorHandlerRemoveBadInseeCodeOverlapAfterDownloadInseeCog
if TYPE_CHECKING:
    from ..requests import RequestCOG
    from ..config import InseeCodeOverlapInseeExceptionsToIgnoreOrCorrect

class CheckInseeCodeOverlapAfterDownloadInseeCog(DataValidationAndConsistencyInseeCog):
    def __init__(
            self,
            insee_code_overlap_exceptions: InseeCodeOverlapInseeExceptionsToIgnoreOrCorrect
        ):
        super().__init__()
        self.insee_code_overlap_exceptions = insee_code_overlap_exceptions

    def handle_exceptions(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection):
        """Handle exceptions in date consistency checks"""
        template_path = Path(__file__).parent.parent / "sql" / "insee_code_overlap_correct.mustashe.sql"
        ...
        uri_updates = [exception for exception in self.insee_code_overlap_exceptions.root if isinstance(exception, ErrorHandlerReplaceBadInseeCodeOverlapAfterDownloadInseeCog)]
        uri_removals = [exception for exception in self.insee_code_overlap_exceptions.root if isinstance(exception, ErrorHandlerRemoveBadInseeCodeOverlapAfterDownloadInseeCog)]
        ...
        context: dict[str, str] = {
            ...
            "modif_start_date_sql": modif_start_date_sql,
            "modif_end_date_sql": modif_end_date_sql,
    ...
    def run(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection, allow_exceptions: bool = False) -> bool:
        ...
        try:
            ...
            if len(data_bug) > 0:
                ...
                if allow_exceptions and (uri_a_bug in [exception.uri for exception in self.insee_code_overlap_exceptions.root] or uri_b_bug in [exception.uri for exception in self.insee_code_overlap_exceptions.root]):
            ...
            raise RuntimeError(f"Unexpected error while checking INSEE code overlap of {request.description} after downloading") from e
