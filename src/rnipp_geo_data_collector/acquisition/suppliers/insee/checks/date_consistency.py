...
from .error_handler import ErrorHandlerReplaceInconsistentDateAfterDownloadInseeCog, ErrorHandlerRemoveInconsistentDateAfterDownloadInseeCog

if TYPE_CHECKING:
    from ..requests import RequestCOG
    from ..config import DateConsistencyInseeExceptionsToIgnoreOrCorrect
...
class CheckDateConsistencyAfterDownloadInseeCog(DataValidationAndConsistencyInseeCog):
    def __init__(
            self,
            date_consistency_exceptions: DateConsistencyInseeExceptionsToIgnoreOrCorrect
        ):
        super().__init__()
        self.date_consistency_exceptions = date_consistency_exceptions

    def handle_exceptions(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection):
        """Handle exceptions in date consistency checks"""
        template_path = Path(__file__).parent.parent / "sql" / "date_consistency_correct.mustashe.sql"
        ...
        uri_updates = [exception for exception in self.date_consistency_exceptions.root if isinstance(exception, ErrorHandlerReplaceInconsistentDateAfterDownloadInseeCog)]
        uri_removals = [exception for exception in self.date_consistency_exceptions.root if isinstance(exception, ErrorHandlerRemoveInconsistentDateAfterDownloadInseeCog)]
    ...
    def run(self, request: RequestCOG, duckdb_conn: DuckDBPyConnection, allow_exceptions: bool = False) -> bool:
        ...
        try:
            ...
            if len(data_bug) > 0:
                ...
                if allow_exceptions and uri_bug in [exception.uri for exception in self.date_consistency_exceptions.root]:
